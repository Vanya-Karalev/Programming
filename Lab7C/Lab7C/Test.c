...
void test(char fileName[11]) {
	...
	delStudent(base,"Ivan", "Karalev");
	delStudent(base, "Michell", "Veromeev");
...
void test1(char fileName[12]) {
	...
	stud->socialAllowance = 1;
	char name[50] = "Ivan";
	char surname[50] = "Karal";
	strcpy(stud->name, name);
	strcpy(stud->surname, surname);
	addStudent(base, stud);
	setSocialAllowance(base, "Ivan", "Karal");
	addPetitions(base, "Ivan", "Karal", 5);
	changeAverageScore(base, "Ivan", "Karal", 8.7876);
	changeName(base, "Ivan", "Karal", "Iva");
	changeSurname(base, "Iva", "Karal", "Karalev");
	printStatement(base);
	baseInFile(base);
	showMenu();
	deleteBase(base);
